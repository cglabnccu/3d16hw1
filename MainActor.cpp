...
void MainActor::display() {
    ...
    static char standBMPpath[] = "image/stand.bmp";
    static char walkBMPpath[]  = "image/walk.bmp";
...
void MainActor::changeStateByKeyboard(const KeyEventController & keyEC) {
    ...
    if(keyEC.isKeyStateDown(' ') && position.y == 0) {
        speed.y = 1;
    }
    if(speed.y == 0) {
        ...
        /*****************************
         if(keyEC.isSpecialKeyStateDown(GLUT_KEY_LEFT)) {
            speed.x -= 0.05;
            if(!isLeftDirection) {
                position = Vec3(position.x + nowPixmap->nCols, position.y);
                isLeftDirection = true;
         ...
         if(keyEC.isSpecialKeyStateDown(GLUT_KEY_RIGHT)) {
            speed.x += 0.05;
            if(isLeftDirection) {
                position = Vec3(position.x - nowPixmap->nCols, position.y);
                isLeftDirection = false;
